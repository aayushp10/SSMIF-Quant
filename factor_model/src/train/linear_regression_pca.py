...
class LinearRegression(BaseModel):
    ...
    def train_model(self, predict: PredictData,
        ...
        print("Train Size")
        print(df[filter(lambda c: c != target_column, list(df.columns))].shape)
        X_train, X_test, y_train, y_test = train_test_split(
            df[[f"predictor_{i}" for i in range(0, self.pc)]], target_df, shuffle=False)
    ...
    def predict(self, data: Optional[List[float]] = None, window: Optional[Window] = None) -> Tuple[List[float], float]:
        ...
        df, target_dt = self.scale_reduce_dimension(data)
